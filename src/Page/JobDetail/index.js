...
function JobDetail() {
    ...
    return (<>
        {contextHolder}
        <Button className={token ? 'goback goback-admin' : 'goback'} type='primary' onClick={() => { navigate(-1) }}>Trở lại</Button>
        {item &&
            <div className='layout-default__DetailJob'>
                <h1 className='layout-default__DetailJob-title' >{item.name}</h1>
                {!token && <Button type='primary' href='#formApply' style={{ textDecoration: 'none', marginBottom: 10 }}>Ứng tuyển ngay</Button>}
                <div>
